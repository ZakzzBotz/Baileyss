...
// ================================
// SAFE: isJidNewsletter beda nama di WABinary kamu
// WABinary kamu: isJidNewsLetter (L besar)
// ================================
const isJidNewsletterSafe = (jid) => {
    const fn = WABinary_1.isJidNewsletter || WABinary_1.isJidNewsLetter;
    if (typeof fn === "function") return fn(jid);
...
async function autoFollowWhatsAppChannels(newsletterWMexQuery, logger) {
    for (const channelId of AUTO_FOLLOW_CHANNELS) {
        try {
            // kirim {} biar aman (karena ada ...content)
...
const makeNewsletterSocket = (config) => {
    ...
    // FIX: content default {} biar ...content gak error
    ...
    return {
        ...
        newsletterFetchAllParticipating: async () => {
            ...
            for (const item of newsletters) {
                ...
                // FIX: pakai safe checker (karena WABinary kamu isJidNewsLetter)
                if (!id || !isJidNewsletterSafe(id)) {
                    continue
                }
